...
impl Client {
    ...
    pub fn send_start(&self, announcer: &Announcer) {
        self.send_event(
    ...
    pub fn send_update(&self, announcer: &Announcer) {
        self.send_event(
            &announcer.announce_url,
            Event::EMPTY,
            &announcer.info_hash,
    ...
    pub fn send_stop(&self, announcer: &Announcer) {
        self.send_event(
            &announcer.announce_url,
            Event::STOPPED,
            &announcer.info_hash,
    ...
    fn send_event(&self, url: &str, event: Event, info_hash: &str, uploaded: usize) {
        let mut parameters = self.create_parameters(info_hash, uploaded);
        match event {
    ...
    fn create_parameters(&self, info_hash: &str, uploaded: usize) -> String {
        format!("?info_hash={}&peer_id={}&port={}&uploaded={}&downloaded=0&left=0&numwant=80&key={}&compact=1&supportcrypto=1",
    info_hash, self.peer_id, self.port, uploaded, self.key)
    }
}
...
impl Announcer {
    pub fn new(torrent: Torrent) -> Announcer {
